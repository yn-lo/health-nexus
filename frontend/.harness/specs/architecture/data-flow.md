...
- 各业务模块（auth、base、chat、wiki、staffChat、config）按领域拆分 API 文件
...
用户登录（shared/views/Login.vue — 统一登录页 /login）
  → shared/api/auth.ts 发起统一登录请求（POST /api/auth/login）
    → shared/api/client.ts 持久化令牌到 localStorage（setTokens）+ stores/auth.ts 持有用户信息
...
- 令牌的写入/读取由 `shared/api/client.ts` 统一管理（localStorage），`stores/auth.ts` 仅持有用户信息
...
- `stores/auth.ts`：全局认证状态，持有用户信息与登录/登出动作（令牌读写见 `shared/api/client.ts`）
- `stores/chat.ts`：聊天会话状态，包括会话列表、当前消息（SSE 流式状态由 `chat/composables/useSSEChat.ts` 管理，不入 store）
