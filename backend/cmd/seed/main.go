...
// 种子科室 ID（与迁移 seed 数据一致）。
const (
	deptCardiology    int64 = 4
	deptEndocrinology int64 = 423
)
...
## 二、甲亢的症状与治疗
甲亢的典型症状包括心悸、手抖、多汗、体重下降、焦虑易怒、眼球突出等。
实验室检查表现为TSH降低、FT3和FT4升高。治疗方法包括抗甲状腺药物（甲巯咪唑、丙硫氧嘧啶）、放射性碘131治疗和手术治疗。药物治疗疗程通常为1.5-2年，不可擅自停药。

## 三、甲减的症状与治疗
甲减表现为乏力、怕冷、体重增加、便秘、皮肤干燥、记忆力减退等。实验室检查表现为TSH升高、FT4降低。
最常见的病因是桥本甲状腺炎。治疗以左甲状腺素（优甲乐）替代治疗为主，需终身服药。服药应在早餐前30-60分钟空腹服用，避免与钙剂、铁剂同服。
...
func post(path, token string, body any) (data []byte, status int) {
...
func login(username string) string {
	...
	if err := json.Unmarshal(data, &r); err != nil {
		fmt.Fprintf(os.Stderr, "parse login resp: %v\n", err)
		os.Exit(1)
	}
	return r.Access
}
...
func createArticle(token, title, content string, deptID int64) int64 {
	...
	var r articleResp
	if err := json.Unmarshal(data, &r); err != nil {
		fmt.Fprintf(os.Stderr, "parse article resp: %v\n", err)
		os.Exit(1)
	}
	return r.ID
}
...
func submitApprove(token string, ids []int64) {
	for _, id := range ids {
		if _, code := post(fmt.Sprintf("/api/staff/wiki/articles/%d/submit", id), token, nil); code != http.StatusOK {
	...
	for _, id := range ids {
		if _, code := post(fmt.Sprintf("/api/staff/wiki/articles/%d/approve", id), token, nil); code != http.StatusOK {
...
// seedDept 为指定科室创建文章并提交审批，返回文章 ID 列表。
func seedDept(token string, deptID int64, seeds []articleSeed) []int64 {
	fmt.Printf("=== Creating dept %d articles ===\n", deptID)
	ids := make([]int64, 0, len(seeds))
	for _, s := range seeds {
		id := createArticle(token, s.title, s.content, deptID)
		fmt.Printf("Created: [%d]\n", id)
		ids = append(ids, id)
	}
	submitApprove(token, ids)
	return ids
}
...
func main() {
	...
	ids1 := seedDept(token1, deptCardiology, []articleSeed{
		{title: "高血压患者日常注意事项", content: art1},
		{title: "冠心病预防与康复指导", content: art2},
	})

	ids2 := seedDept(token2, deptEndocrinology, []articleSeed{
		{title: "糖尿病患者饮食管理指南", content: art3},
		{title: "甲状腺功能异常科普", content: art4},
	})
