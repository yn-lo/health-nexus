...
func TestApplyMMR(t *testing.T) {
	// 构造测试用 fused 切片：3 条内容，其中 doc1 和 doc2 内容相似
	makeFused := func() []fusedHit {
		return []fusedHit{
			{ChunkSearchHit: makeHit(1, 0.9, "感冒 发烧 咳嗽 治疗", "t1"), VecScore: 0.9, RRFScore: 0.5},
			{ChunkSearchHit: makeHit(2, 0.8, "感冒 发烧 咳嗽 预防", "t2"), VecScore: 0.8, RRFScore: 0.4},
			{ChunkSearchHit: makeHit(3, 0.7, "高血压 糖尿病 管理", "t3"), VecScore: 0.7, RRFScore: 0.3},
		}
	}

	t.Run("DiversityFactor为0_保持原顺序", func(t *testing.T) {
		fused := makeFused()
		got := applyMMR(fused, 0.0)
		if len(got) != 3 {
			t.Fatalf("期望 3 条，实际 %d", len(got))
		}
		// λ=0 时纯相关性排序，应保持原 RRF 顺序
		if got[0].ID != 1 || got[1].ID != 2 || got[2].ID != 3 {
			t.Errorf("期望 IDs 1,2,3，实际 %d,%d,%d", got[0].ID, got[1].ID, got[2].ID)
		}
	})

	t.Run("DiversityFactor为1_最大化多样性", func(t *testing.T) {
		fused := makeFused()
		got := applyMMR(fused, 1.0)
		if len(got) != 3 {
			t.Fatalf("期望 3 条，实际 %d", len(got))
		}
		// λ=1 时纯多样性：第一条仍是最高分（id=1），
		// 第二条应选与 id=1 最不相似的（id=3，高血压主题），而非 id=2（相似感冒主题）
		if got[0].ID != 1 {
			t.Errorf("期望首条 ID=1（最高 RRF），实际 %d", got[0].ID)
		}
		if got[1].ID != 3 {
			t.Errorf("期望第二条 ID=3（与首条最不相似），实际 %d", got[1].ID)
		}
	})

	t.Run("DiversityFactor为0.5_平衡相关性与多样性", func(t *testing.T) {
		fused := makeFused()
		got := applyMMR(fused, 0.5)
		if len(got) != 3 {
			t.Fatalf("期望 3 条，实际 %d", len(got))
		}
		// 平衡模式下，第一条仍是最高分
		if got[0].ID != 1 {
			t.Errorf("期望首条 ID=1，实际 %d", got[0].ID)
		}
	})

	t.Run("空输入_空输出", func(t *testing.T) {
		got := applyMMR(nil, 0.5)
		if len(got) != 0 {
			t.Errorf("期望空切片，实际 %d", len(got))
		}
	})

	t.Run("单条输入_原样返回", func(t *testing.T) {
		fused := []fusedHit{
			{ChunkSearchHit: makeHit(1, 0.9, "doc1", "t1"), VecScore: 0.9, RRFScore: 0.5},
		}
		got := applyMMR(fused, 0.5)
		if len(got) != 1 || got[0].ID != 1 {
			t.Errorf("期望单条 ID=1，实际 %v", got)
		}
	})
}
